...
class IngestManager:
    ...
    async def fetch_vector_config(self):
        try:
            ...
            for org in self.orgIds:
                logger.info(f"Fetching vector config for org: {org}")
                organizationDB = OrganizationDataBase(org)
                config, status_code = organizationDB.get_vector_config()

                if status_code == 200 and config:
                    result.extend(config)
                    logger.info(f"Vector config retrieved for org: {org}")
                else:
                    logger.warning(f"No vector config found for org: {org}")

            if not result:
                return {
                    "status_code": status.HTTP_404_NOT_FOUND,
                    "detail": "No vector config found for any org"
                }

            return {
                "status_code": status.HTTP_200_OK,
                "detail": "Vector config retrieved successfully",
                "data": result
            }
            ...
            return {
                "status_code": status.HTTP_500_INTERNAL_SERVER_ERROR,
                "detail": f"Error fetching vector config: {str(e)}"
            }

    async def fetch_embedding_models(self):
        try:
            result = []

            for org in self.orgIds:
                logger.info(f"Fetching embedding models for org: {org}")
                organizationDB = OrganizationDataBase(org)
                models, status_code = organizationDB.get_embedding_models()

                if status_code == 200 and models:
                    result.extend(models)
                    logger.info(f"Embedding models retrieved for org: {org}")
                else:
                    logger.warning(f"No embedding models found for org: {org}")

            if not result:
                return {
                    "status_code": status.HTTP_404_NOT_FOUND,
                    "detail": "No embedding models found for any org"
                }

            return {
                "status_code": status.HTTP_200_OK,
                "detail": "Embedding models retrieved successfully",
                "data": result
            }
            ...
            return {
                "status_code": status.HTTP_500_INTERNAL_SERVER_ERROR,
                "detail": f"Error fetching embedding models: {str(e)}"
            }

    async def fetch_splitter_config(self):
        try:
            result = []

            for org in self.orgIds:
                logger.info(f"Fetching splitter config for org: {org}")
                organizationDB = OrganizationDataBase(org)
                splitters, status_code = organizationDB.get_splitter_config()

                if status_code == 200 and splitters:
                    result.extend(splitters)
                    logger.info(f"Splitter config retrieved for org: {org}")
                else:
                    logger.warning(f"No splitter config found for org: {org}")

            if not result:
                return {
                    "status_code": status.HTTP_404_NOT_FOUND,
                    "detail": "No splitter config found for any org"
                }

            return {
                "status_code": status.HTTP_200_OK,
                "detail": "Splitter config retrieved successfully",
                "data": result
            }
            ...
            return {
                "status_code": status.HTTP_500_INTERNAL_SERVER_ERROR,
                "detail": f"Error fetching splitter config: {str(e)}"
            }
