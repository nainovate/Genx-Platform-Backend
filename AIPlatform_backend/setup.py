...
def connectToSharepoint(cloud_config, timestamp):
        try:
...
def ingestService(api_url: str, input_data: dict, organizationDB):
        ...
        try:
            ...
            status_code, job = organizationDB.checkJob(jobId=jobId)
            timestamp = job["prev_job"][0] if job.get("prev_job") else None
            args= connectToSharepoint(cloud_config=config_data["cloud_config"],timestamp=timestamp)
            if args.get("skip"):
                pass
            else:
