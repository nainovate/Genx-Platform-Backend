...
def check_existing_user(user_setup, username: str, email: str):
    try:
        if not isinstance(user_setup, UsersSetup):
            raise TypeError("user_setup should be an instance of UsersSetup.")
...
def create_user(user_setup, user_data: dict):
    try:
        if not isinstance(user_setup, UsersSetup):
            raise TypeError("user_setup must be an instance of UsersSetup.")
    ...
    try:
        ...
        if not demo:
            ...
            if register_config is not None:
                ...
                collections_initialized, status_code = setup_users_db()
